<script>
  ...
  import { pick_random, load_image } from "../utils";
  ...
  const pick_message = (p) => {
    if (p < 0.5)
      return pick_random(["Ouch", "That wasn\t very good", "Must try harder"]);
    if (p < 0.8) return pick_random(["Not bad", "Keep practising"]);
...
<div class="game-container">
  {#if done}
    <div
      class="done"
      in:scale={{ delay: 200, duration: 8000, easing: eases.elasticOut }}>
